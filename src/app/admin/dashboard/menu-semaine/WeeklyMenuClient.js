...
import { Pencil, Trash2, Plus, X, Image as ImageIcon, CalendarDays, CheckCircle2 } from 'lucide-react';
    ...
    return (
        <div className="animate-fade" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
            <div style={{ width: '100%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '2.5rem' }}>
                <h1 style={{ fontSize: '1.8rem', fontWeight: '700', color: '#1e293b', marginBottom: '0.5rem' }}>Menu de la Semaine</h1>
                <p style={{ color: '#64748b', maxWidth: '500px', marginBottom: '1.5rem' }}>
                    Uploadez l&apos;image de votre menu hebdomadaire. Le menu marqué comme &quot;en cours&quot; s&apos;affichera sur la page d&apos;accueil.
                </p>
                <button
                ...
                >
                    {isAdding ? <><X size={16} /> Annuler</> : <><Plus size={16} /> Nouveau Menu</>}
                </button>
            </div>
                ...
                <div className="admin-card" style={{ width: '100%', maxWidth: '700px', marginBottom: '3rem', borderTop: '4px solid var(--admin-primary)' }}>
                    ...
                    <form action={addWeeklyMenu} onSubmit={() => setIsAdding(false)} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        <div>
                            ...
                            <input type="text" name="title" className="admin-input" placeholder="Ex: Menu du 21 au 25 Juillet" required style={{ background: '#faf8f5' }} />
                        </div>

                        <div>
                            <label className="admin-label">Description (optionnel)</label>
                            <textarea name="description" className="admin-input" rows="3" placeholder="Ex: Couscous maison, Tajine d'agneau, Salade du jardin..." style={{ background: '#faf8f5' }}></textarea>
                        </div>

                        <div className="admin-dropzone" style={{ position: 'relative' }}>
                            <label htmlFor="menu-file-new" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%', cursor: 'pointer', padding: '1.5rem 0' }}>
                                <ImageIcon size={40} style={{ marginBottom: '1rem', color: '#94a3b8' }} />
                                <span style={{ display: 'block', color: '#475569', fontWeight: '600', marginBottom: '0.5rem' }}>
                                    Cliquez pour ajouter l&apos;image du menu
                                </span>
                                <span style={{ fontSize: '0.8rem', color: '#94a3b8' }}>
                                    Format Instagram recommandé (carré ou portrait)
                                </span>
                                <input id="menu-file-new" type="file" name="image_file" accept="image/*" style={{ opacity: 0, position: 'absolute', inset: 0, width: '100%', height: '100%', cursor: 'pointer' }} />
                            </label>
                        </div>

                        <div>
                            <label className="admin-label">Ou coller un lien d&apos;image</label>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '1rem', background: '#f0fdf4', borderRadius: '12px', border: '1px solid #bbf7d0' }}>
                            ...
                            <label htmlFor="is_current_new" style={{ fontWeight: '600', color: '#166534', cursor: 'pointer' }}>
                                ✅ Définir comme menu de la semaine en cours (affiché sur la page d&apos;accueil)
                            </label>
                        </div>
            ...
            <div style={{ width: '100%' }}>
                ...
                {menus.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '4rem 2rem', color: '#94a3b8', fontStyle: 'italic', background: 'white', borderRadius: '20px', border: '1px solid #f1ede8' }}>
                        Aucun menu publié pour le moment. Ajoutez votre premier menu !
                    </div>
                ) : (
                            ...
                            editingId === menu.id ? (
                                ...
                                <div key={menu.id} className="admin-card" style={{ gridColumn: '1 / -1', borderTop: '4px solid var(--admin-accent)' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                                        <h2 style={{ fontSize: '1.2rem', fontWeight: '600', color: '#1e293b' }}>Modification en cours</h2>
                                        <button type="button" onClick={() => setEditingId(null)} style={{ color: '#94a3b8', cursor: 'pointer', background: '#f1f5f9', padding: '8px', borderRadius: '50%' }}>
                                            <X size={20} />
                                        </button>
                                    </div>
                                    <form action={editWeeklyMenu} onSubmit={() => setEditingId(null)} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                                        ...
                                        <div className="admin-dropzone" style={{ position: 'relative', padding: '1.5rem' }}>
                                            <label htmlFor={`menu-file-${menu.id}`} style={{ display: 'block', width: '100%', cursor: 'pointer', textAlign: 'center' }}>
                                                <span style={{ fontWeight: '600', color: '#475569' }}>Remplacer l&apos;image</span>
                                                <input id={`menu-file-${menu.id}`} type="file" name="image_file" accept="image/*" style={{ opacity: 0, position: 'absolute', inset: 0, width: '100%', height: '100%', cursor: 'pointer' }} />
                                            </label>
                                        </div>
                                        <div>
                                            <label className="admin-label">Ou nouveau lien</label>
                                            <input type="url" name="image_url" className="admin-input" placeholder="Laissez vide pour conserver l'image actuelle" style={{ background: '#faf8f5' }} />
                                        </div>
                                ...
                                <div key={menu.id} className="admin-card" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', transition: 'all 0.3s ease', position: 'relative' }}>
                                    ...
                                    <div className="admin-thumb-menu">
                                        ...
                                        <div style={{ position: 'absolute', top: '12px', right: '12px', display: 'flex', gap: '0.5rem' }}>
                                            <button onClick={() => { setEditingId(menu.id); setIsAdding(false); }} style={{ width: '36px', height: '36px', background: 'rgba(255,255,255,0.95)', borderRadius: '50%', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#475569', border: 'none', transition: 'all 0.2s' }} title="Modifier">
                                                <Pencil size={16} />
                                            </button>
                                            <form action={deleteWeeklyMenu}>
                                                <input type="hidden" name="id" value={menu.id} />
                                                <button type="submit" style={{ width: '36px', height: '36px', background: 'rgba(255,255,255,0.95)', borderRadius: '50%', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#ef4444', border: 'none', transition: 'all 0.2s' }} title="Supprimer">
                                                    <Trash2 size={16} />
                                                </button>
                                            </form>
                                        </div>
